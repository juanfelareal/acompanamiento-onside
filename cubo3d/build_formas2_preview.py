...
const LBL={huevo:"Huevo",domo:"Domo",trapecio:"Trapecio",bisel:"Bisel"};
const cont=document.getElementById('txbtns');const tb=[];
DATA.bodies.forEach((bd,i)=>{const btn=document.createElement('button');
  btn.className='tx'+(i===0?' on':'');btn.textContent=LBL[bd.name]||bd.name;
  btn.onclick=()=>{cur=i;tb.forEach((b,j)=>b.classList.toggle('on',j===i));};
  cont.appendChild(btn);tb.push(btn);});
